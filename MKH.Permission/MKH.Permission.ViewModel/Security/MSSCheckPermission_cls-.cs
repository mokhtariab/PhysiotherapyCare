    ...
    public class MSSCheckPermission_cls
    {
        public void CheckPermissionControl(Form FormControl)
        {
            ...
            foreach (Control item in FormControl.Controls)
            {
                if (item is MKH.Library.Control.MSSHeaderGrid)
                {
                    foreach (DevComponents.DotNetBar.ButtonItem item1 in (item as MKH.Library.Control.MSSHeaderGrid).Items)
                    {
                        try
                        {
                            ...
                            item1.Enabled = new ViewModel.SoftwareAccessUser.SoftwareAccessUser().GetAllListByID(Convert.ToInt32(item1.Tag), MKH.Permission.ViewModel.Security.MSSConfiguration_cls.Instance.USER.ToInt(), ref str).Count != 0;
                        }
                        catch { }
                    }
                }
                            ...
                            foreach (Control item2 in (item1 as DevComponents.DotNetBar.RibbonPanel).Controls)
                            {
                                try
                                {
                                    if (item2.Tag != null && item2.Tag.ToString() != "" && item2.Tag.ToString() != "0")
                                        item2.Enabled = new ViewModel.SoftwareAccessUser.SoftwareAccessUser().GetAllListByID(Convert.ToInt32(item2.Tag), MKH.Permission.ViewModel.Security.MSSConfiguration_cls.Instance.USER.ToInt(), ref str).Count != 0;
                                }
                                catch { }
                                foreach (DevComponents.DotNetBar.ButtonItem item3 in (item2 as DevComponents.DotNetBar.RibbonBar).Items)
                                {
                                    try
                                    {
                                        if (item3.Tag != null && item3.Tag.ToString() != "" && item3.Tag.ToString() != "0")
                                            item3.Enabled = new ViewModel.SoftwareAccessUser.SoftwareAccessUser().GetAllListByID(Convert.ToInt32(item3.Tag), MKH.Permission.ViewModel.Security.MSSConfiguration_cls.Instance.USER.ToInt(), ref str).Count != 0;
                ...
                if (item is MKH.Library.Control.MSSTabControl)
                {
                    try
                    {
                        if (item.Tag == null || item.Tag.ToString() == "" || item.Tag.ToString() == "0") continue;
                        item.Enabled = new ViewModel.SoftwareAccessUser.SoftwareAccessUser().GetAllListByID(Convert.ToInt32(item.Tag), MKH.Permission.ViewModel.Security.MSSConfiguration_cls.Instance.USER.ToInt(), ref str).Count != 0;
                    }
                    catch { }
                }
        ...
        public void CheckPermissionControl(UserControl FormControl)
        {
            ...
            try
            {
                foreach (Control item in FormControl.Controls)
                {
                    ...
                    //        if (new ViewModel.SoftwareAccessUser.SoftwareAccessUser().GetAllListByID(Convert.ToInt32(q.Tag), MKH.Permission.ViewModel.Security.MSSConfiguration_cls.Instance.USER.ToInt(), ref str).Count != 0)
                    //            q.Tag = null;
                    //    }
                    //}
        ...
        public bool CheckPermissionItem(int ItemTag)
        {
            ...
            string str = "";
            return new ViewModel.SoftwareAccessUser.SoftwareAccessUser().GetAllListByID(ItemTag, MKH.Permission.ViewModel.Security.MSSConfiguration_cls.Instance.USER.ToInt(), ref str).Count != 0;
        }
