...
namespace MKH.Library.Forms
{
    public partial class MSSColumnSelector_frm : Form
    {
        ...
        public MSSColumnSelector_frm(MKH.Library.Control.MSSGridexView GridViewEnter)
        {
            InitializeComponent();
        ...
        {
            DefaulSettingsKey = _grid.SettingsKey;
        ...
        {
            if (File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\ColumnSelector\\" + _grid.Name + (_grid.Tag ?? 0).ToString() + ".xml"))
            {
                using (var reader = new StreamReader(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\ColumnSelector\\" + _grid.Name + (_grid.Tag ?? 0).ToString() + ".xml"))
                {
        ...
        {
            if (!Directory.Exists(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\ColumnSelector"))
                Directory.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\ColumnSelector");

            using (var writer = new FileStream(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\ColumnSelector\\" + _grid.Name + (_grid.Tag ?? 0).ToString() + ".xml", FileMode.Create))
            {
        ...
        private void MSSColumnSelector_frm_FormClosed(object sender, FormClosedEventArgs e)
        {
            _grid.SaveSettings = false;
        }
