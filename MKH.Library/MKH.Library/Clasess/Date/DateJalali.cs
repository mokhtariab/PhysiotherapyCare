...
{
    [Serializable]
    public class DateJalali : Calendar
    {
        private static DateJalali InternalInstance = null;
        public static DateJalali Instance
        {
            get
            {
                if (InternalInstance == null)
                    InternalInstance = new DateJalali();
        ...
        /// <summary>
        /// »« «Ì‰ „ œ „Ì  Ê«‰Ì„  ⁄œ«œ —Ê“ „‘Œ’ ‘œÂ —« »Â Ìò  «—ÌŒ ‘„”Ì «÷«›Â ò‰Ì„
        /// </summary>
        /// <param name="x_Date"></param>
        /// <param name="x_Days"></param>
        /// <returns></returns>
        public string AddDays(string x_Date, double x_Days)
        {
            //DatePicker MyDatePicker = new DatePicker();
            //if (MyDatePicker.IsValidDate(x_Date))
            //{
            //  DateTime MyDateTime = this.ToDateTime(x_Date);
            //  MyDateTime = MyDateTime.AddDays(x_Days);
            //  return (this.GetPersianDate(MyDateTime, true));
            //}
            //else
            //  return string.Empty;
            return "";
        ...
        public string GetNameOfMounth(System.DateTime time)
        {
            int monthIndex = this.GetMonth(time);
        ...
        {
            return GetDaysInMonth(true, year, month, 0);
        }
        ...
        public System.DateTime ToDateTime(string farsiDate)
        {
            int hour, minute, second, millisecond, era = 1;
            int year, month, day;
            if (farsiDate.Trim() == "" || farsiDate.Trim().Length != 10)
                return DateTime.Now;
            string[] strs = farsiDate.Split(new char[] { '/' });
            //			if ( strs.Length !=3 )
            //				return null;
            year = Int32.Parse(strs[0]);
            month = Int32.Parse(strs[1]);
            day = Int32.Parse(strs[2]);
            hour = 12;
            minute = 0;
            millisecond = 0;
            second = 0;
        ...
        /// <summary>
        /// Converts the specified two-digit year to a four-digit year by using the Globalization.FarsiDate.TwoDigitYearMax property to determine the appropriate century.
        /// </summary>
        ...
        /// <summary>
        /// Gets the list of eras in the FarsiDate.
        /// </summary>
        /// <remarks>The Persian calendar recognizes one era: A.P. (Latin "Anno Persarum", which means "the year of/for Persians").</remarks>
        ...
        public string GetPersianDate(DateTime time, bool withSlash, bool fullDate)
        {
            string todDay = GetPersianDate(time, withSlash);
            if (fullDate == false)
                todDay = todDay.Substring(2, todDay.Length - 2);
            return todDay;
        }
        ...
        {
            string strDate;
            //.Â‰ê«„Ì ﬂÂ —Ê“ Ì« „«Â  «—ÌŒ ﬂ„ — «“ 10 »«‘œ 0 ¬‰ —« Ã«Ìê“Ì‰ „Ì ﬂ‰œ 
            if (withSlash)
            {
                strDate = GetYear(time).ToString() + "/";

                if (GetMonth(time) < 10)
                {
                    strDate = strDate + "0" + GetMonth(time).ToString() + "/";
                }
                else
                {
                    strDate = strDate + GetMonth(time).ToString() + "/";
                }
                if (GetDayOfMonth(time) < 10)
                {
                    strDate = strDate + "0" + GetDayOfMonth(time).ToString();
                }
                else
                {
                    strDate = strDate + GetDayOfMonth(time).ToString();
                }
            }
            else
                strDate = GetYear(time).ToString() + GetMonth(time).ToString() + GetDayOfMonth(time).ToString();
            return strDate;
        }
        ...
        #region Zare Custom
        //»«“ „Ì ê—œ«‰œ True „ œ “Ì— œÊ  «—ÌŒ „Ì êÌ—œ «ê—  «—ÌŒ «Ê· «“  «—ÌŒ œÊ„ »“—ê — »«‘œ „ﬁœ«— 
        public bool BigDate(string x_FirstDate, string x_SecondDate)
        {
            int Azyear = Convert.ToInt32(x_FirstDate.Substring(0, 4).ToString());
            int Azmonth = Convert.ToInt32(x_FirstDate.Substring(5, 2).ToString());
            int Azday = Convert.ToInt32(x_FirstDate.Substring(8, 2).ToString());
            int Tayear = Convert.ToInt32(x_SecondDate.Substring(0, 4).ToString());
            int Tamonth = Convert.ToInt32(x_SecondDate.Substring(5, 2).ToString());
            int Taday = Convert.ToInt32(x_SecondDate.Substring(8, 2).ToString());
            if (Azyear < Tayear) return false;
            if (Azyear <= Tayear)
            {
                if (Azmonth == Tamonth)
                {
        ...
        /// <summary>
        ///«Ì‰ „ œ œÊ  «—ÌŒ ‘„”Ì —« ê—› Â Ê  «—ÌŒ «Œ ·«› —«  Ê”ÿ ”Â „ €Ì— Œ—ÊÃÌ »Â „« „Ì œÂœ
        /// </summary>
        /// <param name=" «—ÌŒ ‘—Ê⁄"></param>
        /// <param name=" «—ÌŒ Å«Ì«‰"></param>
        /// <param name="”«· »«“ê‘ Ì"></param>
        /// <param name="„«Â »«“ê‘ Ì"></param>
        /// <param name="—Ê“ »«“ê‘ Ì"></param>
        public void BetweenDate(string x_AzTarigh, string x_TaTarigh, out int ZarfYear, out int ZarfMonth, out int ZarfDay)
        {
            #region Check&RepairDate
            //ﬁ»· «“ Â— çÌ“ ’Õ   «—ÌŒ çò ‘Êœ
            //DatePicker dp = new DatePicker();
            //if (!dp.IsValidDate(x_AzTarigh, out x_AzTarigh) || !dp.IsValidDate(x_TaTarigh, out x_TaTarigh))
            //{
            //    ZarfDay = 0;
            //    ZarfMonth = 0;
            //    ZarfYear = 0;
            ...
            #endregion
            //»«Ìœ ﬁ—«— »êÌ—œ x_TaTarigh  «—ÌŒ »“—ê — Â„Ì‘Â œ— „ ›Ì— 
            #region Jabejaei
            string TempDate;
            if (BigDate(x_AzTarigh, x_TaTarigh))
            {
                TempDate = x_AzTarigh;
                x_AzTarigh = x_TaTarigh;
                x_TaTarigh = TempDate;

            }
            //œ— ’Ê—  Ã«»Â Ã«ÌÌ œÊ  «—ÌŒ œÊ»«—Â ⁄„·  ›òÌò  «—ÌŒ ‘—Ê⁄ Ê  «—ÌŒ Å«Ì«‰ «‰Ã«„ „Ì ‘Êœ
            int Azyear = Convert.ToInt32(x_AzTarigh.Substring(0, 4).ToString());
            int Azmonth = Convert.ToInt32(x_AzTarigh.Substring(5, 2).ToString());
            int Azday = Convert.ToInt32(x_AzTarigh.Substring(8, 2).ToString());
            int Tayear = Convert.ToInt32(x_TaTarigh.Substring(0, 4).ToString());
            int Tamonth = Convert.ToInt32(x_TaTarigh.Substring(5, 2).ToString());
            int Taday = Convert.ToInt32(x_TaTarigh.Substring(8, 2).ToString());
            #endregion
            ...
            {
                if (Tamonth <= 6 && Tamonth > 1) Taday += 31;
                else
                    if (Tamonth < 12 && Tamonth > 6 || Tamonth == 1)
                        Taday += 30;
                    else
                        Taday += 29;
